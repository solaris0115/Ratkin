...
namespace NewRatkin
{
	public static class DebugAutotests
	{
		...
		{
			//간격
			int count = (from pawnKindDef in DefDatabase<PawnKindDef>.AllDefs where pawnKindDef.defaultFactionDef == RatkinFactionDefOf.RK_Faction_Kingdom select pawnKindDef).Count();
			...
			foreach (PawnKindDef pawnKindDef in (from def in DefDatabase<PawnKindDef>.AllDefs where def.defaultFactionDef == RatkinFactionDefOf.RK_Faction_Kingdom select def) )
			{
				if (x >= Map.Size.x)
				{;
					x = interval;
				...
				for (int n=0; n<50;n++)
				{
					Pawn temp = AutoTests_ColonyMaker.MakeRatkinColonists(new IntVec3(x, 0, y), pawnKindDef, Find.FactionManager.AllFactions.First(faction => faction.def == RatkinFactionDefOf.RK_Faction_Kingdom));
		...
		{
			//간격
			int count = (from def in DefDatabase<PawnKindDef>.AllDefs where def.defaultFactionDef == RatkinFactionDefOf.RK_Faction_Kingdom select def).Count();
