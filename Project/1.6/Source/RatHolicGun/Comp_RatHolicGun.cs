...
namespace NewRatkin
{
    ...
    /// <summary>
    /// RatHolic Gun Spooling Aim Hediff용 Comp 속성
    /// 조준 중인 타겟이 변경되면 Hediff 제거
    /// </summary>
    public class HediffCompProperties_RatHolicGunSpoolingAim : HediffCompProperties
    {
        public HediffCompProperties_RatHolicGunSpoolingAim()
        {
            this.compClass = typeof(HediffComp_RatHolicGunSpoolingAim);
        }
    }

    /// <summary>
    /// RatHolic Gun Spooling Aim Hediff용 Comp
    /// 조준 중인 타겟이 변경되면 스스로를 제거
    /// 매 프레임마다 체크 (CompPostTick 사용)
    /// </summary>
    public class HediffComp_RatHolicGunSpoolingAim : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            // 재장전 중이거나 발사 중이면 타겟 변경 체크 안 함 (계속 유지)
            bool isReloadingOrFiring = IsPawnReloadingOrFiring(pawn, ratHolicGun);
            if (isReloadingOrFiring)
            {
                return;
            }
            ...
            // 조준 중이 아니면 타겟 초기화 (다음 조준 시작 시 감지 가능하도록)
            ...
            if (lastAimingTarget.HasValue)
            {
                ...
                // 타겟이 변경되었으면 Hediff 제거 (LocalTargetInfo는 IEquatable 구현)
        ...
        /// <summary>
        /// Pawn이 현재 재장전 중이거나 발사 중인지 확인
        /// 1. 발사 중 (VerbState.Bursting)
        /// 2. 재장전 중 (Stance_Cooldown)
        /// 조준 중(Stance_Warmup)은 제외 (타겟 변경 감지를 위해)
        /// </summary>
        private bool IsPawnReloadingOrFiring(Pawn pawn, ThingWithComps ratHolicGun)
        {
            if (pawn?.stances == null)
            {
                return false;
            ...
            // 2. 발사 중인지 확인 (VerbState.Bursting)
            CompEquippable compEquippable = ratHolicGun.GetComp<CompEquippable>();
            if (compEquippable != null)
            {
                foreach (Verb verb in compEquippable.AllVerbs)
