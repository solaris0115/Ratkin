ThingDef DefName 목록
@Xxx = 랫킨 연구
#Xxx = 림월드 코어의 연구
Xxx + Xxx 와 같이 +로 합쳐진 경우 모두 연구를 마쳐야함

@목공
- RK_Crossbow           석궁

@목공 + @요새화
- RK_Weapon_Arbalest    노포

@목공 + @검과 방패
- RK_LightLance     나무 랜스

@랫킨식 단조
- RK_Dagger         단검
- RK_OneHanded      한손검
- RK_Axe        도끼
- RK_Cleaver    식칼
- RK_Hockey     쟁기        
- RK_Fork       쇠스랑      

@검과 방패
- RK_Mace           메이스      (건물 높은 피해)
- RK_LongSword      장검
- RK_Spear          창

@검과 방패 + @랫킨식 제련
- RK_TwoHanded      양손검      (절단 위주 피해)
- RK_Halberd        할버드      (밸런스 잡힌 무기)
- RK_HeavyLance     헤비 랜스   (고관통 높은피해, 돌진기능, 낮은 회피, 이동속도저하)

  
@목공 + @랫킨 공학
- RK_AutoCrossBow       자동 쇠뇌

@랫킨 공학
- RK_Rifle              소총
- RK_Rifle_line         샷건
  
@관측과 측량
- RK_SniperRifle        망원 소총

@관측과 측량 + @분리 역학
- RK_Weapon_BFR         고양이 사냥 소총    (적중시 뒤로 탄환이 흩뿌려짐)

@챗바퀴 역학
- RK_Weapon_RatHolicGun 쥑이는 총       (발사 간격이 점점 빨라짐)   

@성난쥐 프로젝트
- RK_Weapon_Bolter          핸드 캐논 (한방이 강한 권총)
- RK_PrototypePulseRifle    파동 소총 (랫킨용 원거리 3연발 소총)
- RK_Weapon_ProtoChainSword 체인소드 (다단히트)
- 
@ 검과 방패 + @랫킨식 제련 + @용의 화염 
- RK_Weapon_Gunlance    랜스캐논
- RK_Ammo_WyvernFire    랜스 캐논 탄약

@랫킨 의류
- RK_ApronSkirt         일상복 1
- RK_WorkerWear         일상복 2
- RK_Coif               두건
- RK_Muffler            목도리  (방한↑)
- RK_Cardigan           외투    (방한↑)
- RK_SummerDress        여름옷  (내한↑)
- RK_StrawHat           밀짚모자 (내한↑)
- RK_WoolenHat          털모자
- RK_CrossBack          봇짐

@기능성 의류
- RK_ResearchGlasses    안경    (명중률↑, 연구속도↑,방어력0)
- RK_ResearchGown       실험복  (치료↑↑)
- RK_ExplorerHat        탐험모  (방한↑ 내한↑)
- RK_ExplorerWear       탐험복  (방한↑내한↑ CaravanBonusSpeedFactor↑)
- RK_ChefHat            요리모  (식중독 확률 down)
- RK_ChefSuit           요리복  (조리속도↑)
- RK_Backpack           가방    (운반중량↑↑)

@고급 의류
- RK_GaurdenUniform     원예복  (회피↑)
- RK_WinterRobe         로브    (방한↑↑)
- RK_OrderUniform       제식복  (재장전↑)
- RK_BulletProofHelmet  방탄모  (원거리 명중률↑)

@귀족 의류
- RK_FlatColorCoat      예복1   (거래개선↑)
- RK_FrillOnepiece      예복2   (거래개선↑)
- RK_SistersDerss       성직 예복  (치료↑)
- RK_SistersVeil        성직모
- RK_HairCorsage        코르사주
- RK_RibbonHairBand     리본

@방직 + @귀족 의류
- RK_WhiteCoat          귀족로브1 (소재방어력)
- RK_RoyalCrown         왕관
- RK_RoyalRobe          귀족로브2 (고정방어력+귀족)

@고급 의류 + @기능성 의류
- RK_BattleSuit         야전복  (방탄↑↑과 화염저항↑)
- RK_Mask               마스크1 (항독성↑)
- RK_MaskB              마스크2 (항독성↑)
- RK_OutdoorBackpack    배낭    (운반중량↑↑↑)

@랫킨식 제련
- RK_Plate              갑옷
- RK_PlateHelmA         투구1
- RK_PlateHelmB         투구2
- RK_PlateHelmC         투구3

!드랍 only
- RK_SantaRobe          산타로브 (방한↑↑)
- RK_SantaHat           산타모  (방한↑)
- RK_SantaSack          선물가방    (운반중량↑↑↑↑)
- RK_HeadBand           머리띠  (고통한계↑)
- RK_Sack               짐자루  (운반중량↑↑↑↑)
- RK_Weapon_ProtoFlameChainSword    플라즈마 체인 소드  (다단히트,화염피해)
- RK_MagicWand  폭발막대    (건물 높은 피해)

@목공
- RK_WoodenShield       목재 방패
@랫킨식 단조
- RK_HeavyShield        철제 방패
@검과 방패 + @랫킨식 단조
- RK_TowerShield        대방패      
@검과 방패
- RK_Apparel_Banner     깃발

@고급 의류 + @궤도 장비 + @방직
- RK_Apparel_VacsuitHelmet          진공헬멧
- RK_Apparel_Vacsuit                진공복
- RK_Apparel_VacsuitChildren        어린이 진공복

@크루세이더 갑옷
- RK_Apparel_SpaceArmor             크루세이더 갑옷
- RK_Apparel_SpaceArmorHelmet       크루세이더 투구

@식품 보존
- RK_Food_Hardtack      건빵
- RK_StrawberryBeer     딸기 맥주   (던질 수 있음)

#Bionics
- RK_Thing_BionicTail

@요새화
- RK_Turret_Ballista_Strait
- RK_Building_Cannon
- BallistaBolt_Normal
- 트리뷰셋

@요새화 + @분리 역학
- BallistaBolt_Heavy    (주변으로 파편이 튐)

@요새화 + @랫킨식 제련
- BallistaBolt_AP       (확률적 관통)

!무시
- RK_Weapon_FlameChainSword         #미사용
- RK_Weapon_EMPChainSword           #미사용