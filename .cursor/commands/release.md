...
1. **빌드**: 프리릴리스와 동일 — MSBuild **`/t:Build`** 만, `Configuration=Release`, `RatkinDevFeatures=false` → `Assemblies/NewRatkin.dll` (`tools/release.py`와 동일 인자)
...
**로컬 컴파일만**(ZIP·GitHub 없음)은 **`/build-release`** / **`/build-dev`** — 둘 다 `Build` 타깃만, 속성만 다름.
...
- `Project/1.6/Source` — MSBuild **`/t:Build`**, Release, `RatkinDevFeatures=false` → `Assemblies/NewRatkin.dll` 반영
