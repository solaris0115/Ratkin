...
3. **ZIP 패키징**: `Project/` 전체 압축, `Project/1.5/` 제외
4. **복사**: 대괄호 없는 이름으로 복사 (gh glob 오류 방지)
...
$filename = "[1.6]TestBuild_${date}_${version}.zip"
...
# 4. 복사 (gh glob 오류 방지)
Copy-Item -LiteralPath $output -Destination "Build/TestBuild/Ratkin_TestBuild_${version}.zip" -Force

# 5. GitHub Pre-release 업로드 (NewRatkin)
gh -R solaris0115/NewRatkin release delete Dev1.6 --yes
gh -R solaris0115/NewRatkin release create Dev1.6 "Build/TestBuild/Ratkin_TestBuild_${version}.zip" --title "[1.6]TestBuild" --prerelease -n " "
```
