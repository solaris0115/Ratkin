---
description: 프리릴리스 배포 — MSBuild Rebuild·ZIP·GitHub Pre-release(Dev1.6)까지 (로컬만은 /build-release·/build-dev)
---
...
`Configuration=Release`이면 `RatkinDevFeatures`는 기본 `false`이며 `RATKIN_DEV_FEATURES`가 정의되지 않는다. `NewRatkin.csproj`의 `OutputPath`가 `..\Assemblies\`이므로 **`/t:Rebuild`** 로 `Project/1.6/Assemblies/NewRatkin.dll`에 반영된다(증분 건너뜀 없음). **`bin\Release\NewRatkin.dll`은 더 이상 갱신되지 않으므로 Assemblies로 복사하면 안 된다**(오래된 DLL로 덮여 타입 누락 오류가 난다).
