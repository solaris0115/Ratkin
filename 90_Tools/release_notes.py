#!/usr/bin/env python3
"""
Daily / 릴리즈 노트: 80_ReleaseNote 에 일별 커밋 기록 후, Daily 를 모아 릴리즈 노트 초안 생성.

저장소 루트에서:
  python 90_Tools/release_notes.py append-daily    # HEAD 커밋을 해당 일 Daily 파일에 추가
  python 90_Tools/release_notes.py create-release    # 모든 *_DAILY.md 를 읽어 실행일 기준 *_RELEASE_NOTES_DRAFT.md 작성 (해시·시각 메타는 넣지 않음)
"""
...
def create_release(exec_date: str, repo: Path = ROOT, out_path: Path | None = None) -> Path:
    ...
    parts: list[str] = [
        ...
        "",
        "> 이 파일은 `*_DAILY.md` 기록을 바탕으로 `90_Tools/release_notes.py create-release` 로 생성·갱신된 초안입니다. 배포 전 문구·분류를 다듬으세요.",
        "",
        "### 일별 상세 (Daily 원문)",
        "",
    ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="80_ReleaseNote Daily / 릴리즈 노트 도구")
    ...
    p_rel.add_argument(
        ...
        type=Path,
        default=None,
        help="출력 파일 경로 (상대 시 저장소 루트 기준). 미지정 시 80_ReleaseNote/YY.MM.DD_RELEASE_NOTES_DRAFT.md",
    )
